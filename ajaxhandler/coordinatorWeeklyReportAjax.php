...
/**
 * Helper function to get proper image URL (local or Cloudinary)
 */
...
function handleImageUploads($files) {
    ...
    foreach ($files['name'] as $index => $name) {
        if ($files['error'][$index] === UPLOAD_ERR_OK) {
            ...
            // Cloud-first upload
            $uploadResult = safeUploadImage($tmpName, $name, 'uploads', 'reports', true);
            if (!empty($uploadResult['success'])) {
                ...
                error_log('Cloud upload failed (handleImageUploads): ' . ($uploadResult['error'] ?? 'unknown error'));
            }
        }
    }
...
function handleImageUploadsPerDay($files) {
    ...
    // Cloud-first uploads handled via safeUploadImage; no local directory needed
            ...
            foreach ($files['images']['name'][$dayKey] as $index => $name) {
                ...
                if ($files['images']['error'][$dayKey][$index] === UPLOAD_ERR_OK) {
                    ...
                    if (!empty($uploadResult['success'])) {
                        ...
                        error_log("Cloud upload successful for $day - URL: {$uploadResult['url']}");
                    } else {
                        error_log("Cloud upload failed for $day: " . ($uploadResult['error'] ?? 'unknown error'));
                    }
                } else {
                ...
                if (isset($image['tmp_name']) && !empty($image['tmp_name'])) {
                    try {
                        ...
                        if (!empty($uploadResult['success'])) {
                            ...
                            error_log("Cloud upload failed for day: $day - " . ($uploadResult['error'] ?? 'unknown error'));
                            continue;
                        }
                    } catch (Exception $e) {
